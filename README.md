...
> **v26.1.0-alpha 起由 C#/WinForms 重构为 Flutter + Material Design 3**：全新 MD3 组件体系（动态配色、菜单、对话框、进度指示），浅色 / 深色主题跟随系统，正文保留衬线书卷排版。本版本为预览版，旧版 C# 实现保留在仓库根目录。

- 单一程序包，无需安装任何运行时
- 支持 EPUB 2 / EPUB 3
- 自动解析目录（NCX / nav.xhtml），章节间切换
- 纯 Flutter 原生渲染章节内容（标题/段落/引用/列表/表格/代码块/图片），不依赖 WebView
- Material Design 3 界面，浅色 / 深色主题跟随系统（v26.1.0-alpha）
